...
#define NTESTS 100 
  ...
  #define UART_INIT() ;
    ...
    //#define PRINT_MEDIAN() print_one(&tests[1]);
...
int main(void)
{
    ...
    printf("**************************************\n");
    printf("****** PQCUARK Speed Test *******\n");
    printf("**************************************\n");
    #ifdef DILITHIUM_RANDOMIZED_SIGNING
        printf("Randomized signing is ON; disable it for deterministic benchmarking.\n");
        printf("Randomizing seed...");
        randombytes(seed, SEEDBYTES);
        printf("DONE\n");
    #else
        printf("Using deterministic seed...");
        seed_randombytes(seed, SEEDBYTES, 123);
        printf("DONE\n");
    #endif
    ...
    for (i = 0; i < NTESTS; ++i) {
        CPU_PERF_START(i);
        poly_uniform(&mat[0].vec[i], seed, 123);
        CPU_PERF_END(i);
    }
    printf("poly_uniform:\n");
    PRINT_MEDIAN();
...
*/
    ...
    for (i = 0; i < NTESTS*10; ++i) {
        CPU_PERF_START(i);
        rej = crypto_sign_signature(sig, &siglen, sig, CRHBYTES, sk);
        CPU_PERF_END(i);
    }
    printf("Sign with rand:\n");
    PRINT_MEDIAN();

    #ifdef DILITHIUM_COUNT_REJ_NUM
