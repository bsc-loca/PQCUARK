...
int main() {
  ...
  int32_t zeta_x = zetas[2];

  printf("Initial values: x1=%x, x2=%x, zeta_x=%x\n", x1, x2, zeta_x);

  // Expected outputs
  uint32_t expected_x1 = x1, expected_x2 = x2;

  intt_butterfly(&expected_x1, &expected_x2, zeta_x);

  printf("Expected outputs: x2=%x, x1=%x\n", expected_x2, expected_x1);
  printf("expected_x2: %016lx\n", (int64_t) expected_x2 << 32);
  printf("expected_x1: %016lx\n", (int64_t) expected_x1);
  int64_t concatenated = (((int64_t) expected_x2 << 32) | (uint64_t) expected_x1);
