...
void print_poly(const char *label, const poly *p) {
    printf("%s:\n", label);
    for (int i = 0; i < KYBER_N; i++) {
        printf("%d ", p->coeffs[i]);
        if ((i + 1) % 16 == 0) printf("\n");  // Imprimir 16 coeficientes por línea
    }
    printf("\n");
}

// Función para reducir modulo KYBER_Q
int16_t mod_q(int32_t a) {
    a %= KYBER_Q;
    if (a < 0) a += KYBER_Q;
    return (int16_t)a;
...
// Función que multiplica dos polinomios en el dominio de la NTT
void poly_multiply_in_ntt_domain(poly *result, const poly *a, const poly *b) {
    for (int i = 0; i < KYBER_N; i++) {
        result->coeffs[i] = mod_q((int32_t)a->coeffs[i] * b->coeffs[i]);
    }
}

// Verificar que NTT cumple con la propiedad de multiplicación
void test_ntt_multiplication_property() {
    poly a;

    // Inicializa los polinomios a y b con valores de ejemplo
    a.coeffs[0] = 1;
    b.coeffs[0] = 1;
    for (int i = 1; i < KYBER_N; i++) {
        a.coeffs[i] = 0;
        b.coeffs[i] = 0;
    //     a.coeffs[i] = i + 1;  // Por ejemplo, [1, 2, 3, ..., KYBER_N]
    //     b.coeffs[i] = KYBER_N - i;  // [256, 255, ..., 1]
    }


    // Imprimir antes de la NTT
    print_poly("Polinomio A antes de la NTT", &a);

    // Aplica la NTT a ambos polinomios
    ntt(&a);

    // Imprimir después de la NTT
    print_poly("Polinomio A después de la NTT", &a);


    // Normaliza el resultado de la INTT
    int16_t inverse_256 = 3221;  // Inverso de 256 modulo KYBER_Q
    for (int i = 0; i < KYBER_N; i++) {
        product_freq_domain.coeffs[i] = mod_q(product_freq_domain.coeffs[i] * inverse_256);
    }

      // Imprimir después de la normalización
    print_poly("Producto después de la normalización", &product_freq_domain);

    // Comparar el resultado en el dominio del tiempo con el producto obtenido en el dominio de la NTT
    int correct = 1;
    for (int i = 0; i < KYBER_N; i++) {
        if (product_time_domain.coeffs[i] != product_freq_domain.coeffs[i]) {
            correct = 0;
            break;
        }
    }
    ...
    if (correct) {
        printf("La NTT cumple con la propiedad de multiplicación correctamente.\n");
    } else {
        printf("Error: La NTT no cumple con la propiedad de multiplicación.\n");
    }
}

// Verificar que INTT puede revertir correctamente la NTT
void test_intt_reversibility() {
    poly a, b;

    // Inicializa el polinomio a con valores de ejemplo
    a.coeffs[0] = 1;
    for (int i = 1; i < KYBER_N; i++) {
        a.coeffs[i] = 0;
    //     a.coeffs[i] = i + 1;  // Por ejemplo, [1, 2, 3, ..., KYBER_N]
    ...
    // Guardar una copia del polinomio original
    b = a;

     // Imprimir el polinomio original
    print_poly("Polinomio original", &b);

    // Aplica la NTT al polinomio
    ntt(&a);

    // Imprimir después de la NTT
    print_poly("Polinomio después de la NTT", &a);

    // Aplica la INTT al resultado de la NTT
    intt(&a);

    // Imprimir después de la INTT
    print_poly("Polinomio después de la INTT", &a);

    // Normalizar los coeficientes después de la INTT
    int16_t inverse_256 = 3221;  // Inverso de 256 modulo KYBER_Q
    for (int i = 0; i < KYBER_N; i++) {
        a.coeffs[i] = mod_q(a.coeffs[i] * inverse_256);
    }

    // Imprimir después de la normalización
    print_poly("Polinomio después de la normalización", &a);

    // Comprobar si el polinomio original y el resultado son iguales
    int correct = 1;
    for (int i = 0; i < KYBER_N; i++) {
    ...
    if (correct) {
        printf("La INTT revierte correctamente la NTT.\n");
    } else {
        printf("Error: La INTT no revierte correctamente la NTT.\n");
    }
}
...
int main()
{
    ...
    UART_INIT();
    printf("**************************************\n");
    printf("********** Test NTT RV64IM ***********\n");
    printf("**************************************\n");
    test_ntt_multiplication_property();
    test_intt_reversibility();
    #ifdef FPGA
    exit(0);
    #endif
