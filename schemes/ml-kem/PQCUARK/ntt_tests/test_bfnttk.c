...
int main() {
  ...
  // Expected outputs
  uint16_t expected_x1 = x1, expected_x2 = x2;
  uint16_t expected_y1 = y1, expected_y2 = y2;

  ntt_butterfly(&expected_x1, &expected_x2, zeta_x);
  ntt_butterfly(&expected_y1, &expected_y2, zeta_y);

  printf("Expected outputs: x2=%x, y2=%x, x1=%x, y1=%x\n", expected_x2, expected_y2, expected_x1, expected_y1);
  printf("expected_x2: %016lx\n", (uint64_t) expected_x2 << 48);
  printf("expected_y2: %016lx\n", (uint64_t) expected_y2 << 32);
  printf("expected_x1: %016lx\n", (uint64_t) expected_x1 << 16);
  printf("expected_y1: %016lx\n", (uint64_t) expected_y1);
  uint64_t concatenated = (((uint64_t) expected_x2 << 48) | ((uint64_t) expected_y2 << 32) | ((uint64_t) expected_x1 << 16) | (uint64_t) expected_y1);
