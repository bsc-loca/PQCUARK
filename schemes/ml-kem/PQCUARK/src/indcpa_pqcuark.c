...
#define NUM_TESTS 1
  ...
  #define UART_INIT();
    ...
    #define PRINT_AVG(); 
    #else
    #define CPU_PERF_START(i) set_checkpoint_start(&tests_indcpa[i]);
    #define CPU_PERF_END(i) set_checkpoint_end(&tests_indcpa[i]);
    #define PRINT_ONE(i) print_one(&tests_indcpa[i]);
    #define PRINT_MEDIAN() print_median(tests_indcpa, NUM_TESTS);
    #define PRINT_AVG() print_all_average(tests_indcpa, NUM_TESTS);
    #define PRINT_ALL_MEDIAN() print_all_median(tests_indcpa, NUM_TESTS);
    #endif
#else
    #define CPU_PERF_START(...);
    #define CPU_PERF_END(...);
    #define PRINT_ONE(...);
    #define PRINT_MEDIAN(...);
    #define PRINT_AVG(...); 
...
checkpoint_t tests_indcpa[NUM_TESTS];
...
static unsigned int rej_uniform_vector(int16_t *r, const uint8_t *buf)
{
    ...
    vl = __riscv_vsetvl_e16m1(128 / 16);
    t0 = __riscv_vle16_v_u16m1(mask_01_t, vl);
    mask_01 = __riscv_vmseq_vx_u16m1_b16(t0, 1, vl);
    ...
    while (ctr <= KYBER_N - 16 && pos <= REJ_UNIFORM_VECTOR_BUFLEN - 24) {
        ...
        vl = __riscv_vsetvl_e16m1(128 / 16);
        g0 = __riscv_vreinterpret_v_u8m1_u16m1(f0);
        g1 = __riscv_vreinterpret_v_u8m1_u16m1(f1);
        g0 = __riscv_vsrl_vx_u16m1_mu(mask_01, g0, g0, 4, vl);
        g1 = __riscv_vsrl_vx_u16m1_mu(mask_01, g1, g1, 4, vl);

        g0 = __riscv_vand_vx_u16m1(g0, mask_12bits, vl);
        g1 = __riscv_vand_vx_u16m1(g1, mask_12bits, vl);

        good0 = __riscv_vmsltu_vx_u16m1_b16(g0, bound, vl);
        good1 = __riscv_vmsltu_vx_u16m1_b16(g1, bound, vl);
        num0 = __riscv_vcpop_m_b16(good0, vl);
        num1 = __riscv_vcpop_m_b16(good1, vl);

        g0 = __riscv_vcompress_vm_u16m1(g0, good0, vl);
        g1 = __riscv_vcompress_vm_u16m1(g1, good1, vl);

        __riscv_vse16_v_u16m1((uint16_t *)&r[ctr], g0, vl);
        ctr += num0;
        __riscv_vse16_v_u16m1((uint16_t *)&r[ctr], g1, vl);
        ctr += num1;
    }
                ...
                int transposed)
{
    ...
    uint8_t buf[GEN_MATRIX_NBLOCKS * XOF_BLOCKBYTES + 2] __attribute__((aligned(32)));
                    ...
                    uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES])
{
    ...
    PRINTF("***************************hash_g Keypair***************************\n");
    CPU_PERF_START(0);
    hash_g(buf, buf, KYBER_SYMBYTES);
    CPU_PERF_END(0);
    PRINT_ONE(0);

    PRINTF("***************************gen_a Keypair***************************\n");
    CPU_PERF_START(1);
    gen_a(a, publicseed);
    CPU_PERF_END(1);
    PRINT_ONE(1);

    PRINTF("***************************polyvec_gen_eta1_skpv_e Keypair***************************\n");
    CPU_PERF_START(2);
                ...
                const uint8_t coins[KYBER_SYMBYTES])
{
    ...
    PRINTF("*************************polyvec_gen_eta1_sp_eta2_ep_epp*************************\n");
    CPU_PERF_START(1);
